...
def main():
    ...
    # _posts フォルダ内を年月階層まで再帰的に探索
    for root, dirs, files in os.walk(post_dir):
        for filename in files:
            if filename.endswith('.md'):
                md_path = os.path.join(root, filename)
                
                # 1. フォルダ階層の抽出 (例: '2026/07')
                ...
                html_content = html_content.replace('{{DATE}}', meta.get('date', ''))
                html_content = html_content.replace('{{CATEGORIES}}', meta.get('categories', ''))
                html_content = html_content.replace('{{TAGS}}', meta.get('tags', ''))
    ...
    # ルート直下の index.html（トップページ）の生成
    index_html = "<html><head><meta charset='UTF-8'><title>Index</title></head><body><h1>記事一覧</h1><ul>"
    for art in sorted(articles_index, key=lambda x: x['d'], reverse=True):
        index_html += f"<li>{art['d']} - <a href='{art['p']}'>{art['t']}</a></li>"
