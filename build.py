...
# ───【完全解決：二重リンク構造分解・解除回路】───
            # 内側に埋め込まれている「拡大用リンク（target="_blank"を含む<a>タグ）」の前後を検知し、
            # その外枠（動画リンク）や内枠（div、img、span）の構造を一切崩さずに、内側の<a>タグだけを消去します。
            post["body_html"] = re.sub(
                r'<a\s+href="([^"]+)"\s*target="_blank"\s*title="[^"]*"\s*style="[^"]*">(<img\s+[^>]+>)</a>',
                r'\2',
                post["body_html"]
            )
            # ───【ここまで修正】───                       
